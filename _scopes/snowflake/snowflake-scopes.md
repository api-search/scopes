...
- /oauth/authorize
description: 'Snowflake''s OAuth scopes do NOT enumerate permissions. There is no `databases:read` or `warehouse:write`. A Snowflake scope selects which ROLE the session runs as; the permissions themselves come from Snowflake RBAC grants attached to that role inside the account. This is the single most important thing for an integrator to understand: narrowing an OAuth scope narrows the role, and the role is where the actual authorization decision lives.'
docs:
...
note: ''
overview: 'Snowflake publishes 5 OAuth 2.0 scopes via the authorizationCode and implicit flows. Scopes are the fine-grained permissions an application requests at authorization time to act against the Snowflake API on a user''s behalf.
...
- description: Snowflake OAuth — Snowflake itself is the authorization server.
  ...
  - authorizationUrl: https://org-account.snowflakecomputing.com/oauth/authorize
    ...
    note: Both endpoints are per-account. The authorization endpoint is a GET that must be opened in an interactive browser; the token endpoint is a POST that exchanges an authorization code or a refresh token for an access token.
    tokenUrl: https://org-account.snowflakecomputing.com/oauth/token-request
  - authorizationUrl: /oauth/authorize
    flow: implicit
    note: Declared in the contract. The implicit flow is deprecated by OAuth 2.1 and by current IETF security best practice; prefer authorizationCode with PKCE.
  name: SnowflakeOAuth
  source: openapi/snowflake-statements-api-openapi.yml
  type: oauth2
- description: External OAuth — an external identity provider (Okta, Microsoft Entra ID, Ping, a custom OIDC IdP) is the authorization server and Snowflake validates the token it issues.
  docs: https://docs.snowflake.com/en/user-guide/oauth-ext-overview
  name: ExternalOAuth
  note: Scope semantics come from the IdP's configuration, not from Snowflake. Snowflake maps the token's subject claim to a Snowflake user and applies that user's roles. Snowflake is the relying party here, not the authorization server — which is also why no /.well-known/openid-configuration is served on a Snowflake host.
  type: oauth2
scope_count: 5
scope_names:
- session:role:<role_name>
- session:role-encoded:<role_name>
- session:role-any
- session:role:PUBLIC
- refresh_token
scopes:
- description: Restricts the access token to a single Snowflake role. The session runs as that role and cannot switch. This is the least-privilege scope form and the one to use for machine and agent credentials.
  flows:
  - authorizationCode
  - implicit
  scope: session:role:<role_name>
- description: Same as session:role but URL-encodes the role name. Use for roles containing reserved characters.
  flows:
  - authorizationCode
  scope: session:role-encoded:<role_name>
- description: Requests a session that can switch its primary role at runtime with USE ROLE, rather than being pinned to one. Requires OAUTH_ANY_ROLE_MODE to be enabled on the security integration.
  flows:
  - authorizationCode
  scope: session:role-any
- description: The concrete instance of session:role that appears in the contract's own example. PUBLIC is the default role every Snowflake user holds; it grants almost nothing on its own.
  flows:
  - authorizationCode
  scope: session:role:PUBLIC
- description: Requested alongside a role scope in the authorization URL. Snowflake then presents the user with the option to consent to offline access and issues a refresh token, so the client can renew without another interactive authorization.
  flows:
  - authorizationCode
  scope: refresh_token
