...
description: 'OAuth 2.0 scopes ElevenLabs publishes in its RFC 8414 authorization-server metadata and
  ...
  the scopes the consent screen asks for when an MCP client or the ElevenLabs CLI connects.


  PROVENANCE NOTE: these scope names are read verbatim from the provider''s discovery

  documents. The per-scope `grants` text is a plain-language reading of the scope name

  checked against the hosted-MCP capability list; ElevenLabs does not publish a prose scope

  reference page, so treat `grants` as our description and the `name` as the provider''s.
...
source_url: ''
source_yaml: "generated: '2026-09-17'\nmethod: probed\nsource: https://api.elevenlabs.io/.well-known/oauth-authorization-server\ndescription: |\n  OAuth 2.0 scopes ElevenLabs publishes in its RFC 8414 authorization-server metadata and\n  repeats in its RFC 9728 protected-resource document for the hosted MCP server. These are\n  the scopes the consent screen asks for when an MCP client or the ElevenLabs CLI connects.\n\n  PROVENANCE NOTE: these scope names are read verbatim from the provider's discovery\n  documents. The per-scope `grants` text is a plain-language reading of the scope name\n  checked against the hosted-MCP capability list; ElevenLabs does not publish a prose scope\n  reference page, so treat `grants` as our description and the `name` as the provider's.\ndocs: null\ndocs_note: no published scopes/permissions reference page was found on elevenlabs.io; the\n  authorization-server metadata is the only machine-readable source.\nauthorization_server: https://api.us.elevenlabs.io\n\
